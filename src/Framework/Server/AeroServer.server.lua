...
local rbxts_include = game:GetService("ReplicatedStorage"):WaitForChild("rbxts_include")

local Promise = require(rbxts_include.Promise)
...
--> Wait for Aero something
_G.WaitForAero = function()
	do
		local heartbeat = game:GetService("RunService").Heartbeat
		repeat heartbeat:Wait() until _G.AeroClient
	end		
end

_G.WaitForAeroPromise = function()
	return Promise.new(function(resolve)
		local heartbeat = game:GetService("RunService").Heartbeat
		local heartbeatConn

		heartbeatConn = heartbeat:Connect(function()
			if _G.AeroClient then
...
function AeroServer:RegisterClientFunction(funcName, func, cacheTTL, canAsync)
	local remoteFunc = Instance.new("RemoteFunction")
	remoteFunc.Name = funcName
	remoteFunc.OnServerInvoke = function(...)
		return func(...)
	end
	remoteFunc:SetAttribute("CanAsync", canAsync)
...
local function LoadService(module, servicesTbl, parentFolder)
	...
	local importedService = require(module)
	if type(importedService["default"]) ~= "table" then
		return error(module.Name .. " has no default class, please add or replace it (to make it simpler when importing services)")
	end

	local service = importedService["default"]
	servicesTbl[module.Name] = service
	
	local clientContents = importedService[module.Name .. "Client"]

	if (type(clientContents) ~= "table") then
		service.Client = {}
	else
		service.Client = clientContents.new(service)
	end
...
local function InitService(service)
	...
	-- Exemptions:
	local clientTableIndexExemptions = {
		Server = "Server",
		__index = "__index",
	}

	-- Client functions:
	local function setupClientFunction(name, contents)
		assert(type(contents["_wrappedFunction"]) == "function", "Make sure you use the ServerFunction!")
		service:RegisterClientFunction(name, contents._wrappedFunction, service._clientCaches[name], contents._asyncEnabled)
	end

	local function setupEventGlobal(contents)
		service:RegisterClientEvent(contents._name)
	end

	for funcName, funcContents in pairs(service.Client) do
		if (type(funcContents) == "table" and clientTableIndexExemptions[funcName] == nil) then
			if (funcContents["_isFunction"]) then
				setupClientFunction(funcName, funcContents)
			else
				setupEventGlobal(funcContents)
			end
		end
	end
