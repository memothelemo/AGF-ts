...
local Promise = require(rbxts_include:WaitForChild("Promise"))
...
local function LoadService(serviceFolder, servicesTbl)
	...
	for _,v in ipairs(serviceFolder:GetChildren()) do
		...
		elseif (v:IsA("RemoteFunction")) then
			...
			-- anti-exploitable
			local asyncAttribute = v:GetAttribute("CanAsync")
			if (cacheTTL) then
				...
				service[v.Name] = function(...)
					if (asyncAttribute == true) then
						local args = {...}
						return Promise.new(function(resolve)
							resolve(v:InvokeServer(unpack(args)))
						end)
					end
...
local function LoadController(module, controllersTbl)
	...
	local importedController = require(module)

	local controller = importedController["default"]

	if type(importedController) ~= "table" then
